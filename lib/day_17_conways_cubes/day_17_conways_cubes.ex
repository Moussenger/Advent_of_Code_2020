defmodule AdventOfCode.Day17ConwaysCubes do
  ...
  # import AdventOfCode.Day17ConwaysCubes
  # data = ".#. ..# ###"
  # data |> get_active_cubes_after_n_cycles(6, 3)
  # file = "lib/day_17_conways_cubes/input.txt" |> File.read!()
  @doc ~S"""
  ## Examples
    iex> import AdventOfCode.Day17ConwaysCubes
    iex> data = ".#. ..# ###"
    iex> data |> get_active_cubes_after_n_cycles(6, 3)
    112
  """
  def get_active_cubes_after_n_cycles(state, cycles, cube_size) do
    initial_length = 2 * cycles + cube_size
    cycles_length = cycles * 2 + 1

    state
    |> parse_state(cycles, cube_size)
    |> process_n_cycles(cycles, initial_length, cycles_length)
    |> flat_state(initial_length, cycles_length)
    |> List.flatten()
    |> Stream.filter(&(&1 == @active))
    |> Enum.count()
  end

  defp parse_state(state, cycles, cube_size) do
  ...
  defp flat_state(state, initial_length, cycles_length) do
  ...
  defp process_n_cycles(states, 0, _initial_length, _cycles_length), do: states

  defp process_n_cycles(states, cycles, initial_length, cycles_length) do
    states =
      for z <- 0..(cycles_length - 1) do
        for x <- 0..(initial_length - 1) do
          for y <- 0..(initial_length - 1) do
    ...
    process_n_cycles(states, cycles - 1, initial_length, cycles_length)
  end
